...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-2dc6a9eb";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-2dc6a9eb">'+'<span class="tile-face-2dc6a9eb tile-front-2dc6a9eb"></span>'+'<span class="tile-face-2dc6a9eb tile-back-2dc6a9eb"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-2dc6a9eb")){return;}
var back=tile.querySelector(".tile-back-2dc6a9eb");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-2dc6a9eb");if(isMine){tile.classList.add("boom-2dc6a9eb");}
if(dim){tile.classList.add("dimmed-2dc6a9eb");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-2dc6a9eb")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-2dc6a9eb");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-2dc6a9eb")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-2dc6a9eb");void panelEl.offsetWidth;panelEl.classList.add("jolt-2dc6a9eb");}
endRound();}
